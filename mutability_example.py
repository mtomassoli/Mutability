...
# Single-lock version (see "mutability_example2.py" for the multi-lock version)
class A_(Generic[Mut_M, Mut_L]):
    ...
    @overload
    def set_val_overload(self: A_[W, _L], x: int) -> int: ...
    @overload
    def set_val_overload(self: A_[W, _L], x: str) -> str: ...
    ...
    def __imul__(self: A_[W, _L], n: int) -> A_[Mut_M, Mut_L]:
        self._val *= n
        return cast(A_[Mut_M, Mut_L], self)

    # NOTE: these are just for debugging
    def for_r(self): return 0
    def for_w(self: A_[W, _L]): return 0
    def for_rk(self: A_[RK, _L]): return 0
    def for_wk(self: A_[WK, _L]): return 0
